...
#define PTE_VALID(pte) ((pte | PTE_P) == pte)
...
boot_alloc(uint32_t n)
{
	...
	if ((size_t) (ROUNDUP(n, PGSIZE) + PADDR(nextfree)) > npages * PGSIZE)
...
mem_init(void)
{
	...
	// LAB 3: Your code here.
	boot_map_region(
	        kern_pgdir, UENVS, sizeof(struct Env) * NENV, PADDR(envs), PTE_U);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	// Primeros 256MiB (0x10000000) de memoria fisica
	boot_map_region(kern_pgdir, KERNBASE, 0x10000000, 0, PTE_W);
	//	boot_map_region(kern_pgdir, KERNBASE, 0xFFFFFFFF - KERNBASE, 0, PTE_W); //equivalente
	//
...
//
bool
incluir_en_paginas_libres(struct PageInfo *page)
{
	size_t paddr = page2pa(page);
	if (paddr == 0)
		return false;
	else if (paddr >= IOPHYSMEM && paddr < PADDR(boot_alloc(0)))
		return false;
...
page_init(void)
{
	...
	for (i = 1; i < npages; i++) {
		if (incluir_en_paginas_libres(&pages[i])) {
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!PTE_VALID(pgdir_entry)) {  // Si NO tiene el flag Present
		if (!create)
			return NULL;
	...
	physaddr_t pgtable_paddr =
	        PTE_ADDR(pgdir_entry);        // Dir. fisica de la page table
	pte_t *ptdir = KADDR(pgtable_paddr);  // Dir. virtual de la page table
	return &ptdir[PTX(va)];
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	while (size > 0) {
#ifdef TP1_PSE
		if (size >= PTSIZE && va % PTSIZE == 0 &&
		    pa % PTSIZE == 0) {  // Puedo mapear una large page
	...
	0x003FF000 = 0000 0000 0011 1111 1111 0000 0000 0000
	*/
}
...
{
	pte_t *pte = NULL;
	struct PageInfo *pag = page_lookup(pgdir, va, &pte);
	if (!pag)
		return;
