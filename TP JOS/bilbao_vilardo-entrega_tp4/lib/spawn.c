...
copy_shared_pages(envid_t child)
{
	...
	int r;
	for (void *i = 0; i < (void *) UTOP; i += PGSIZE) {
		if (!(PGOFF(uvpd[PDX(i)]) & (PTE_P)))
			continue;

		int perms = PGOFF(uvpt[PGNUM(i)]);

		if (!(perms & PTE_P) || !(perms & PTE_SHARE))
			continue;

		if ((r = sys_page_map(0, i, child, i, perms & PTE_SYSCALL)) < 0)
			panic("Error al mapear memoria: %e", r);
	}
