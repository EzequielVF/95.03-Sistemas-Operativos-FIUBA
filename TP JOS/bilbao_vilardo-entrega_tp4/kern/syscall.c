...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// LAB 4: Your code here.
	int mask_must = PTE_P | PTE_U;
	int mask_may = PTE_AVAIL | PTE_W;
	if (((perm & mask_must) != mask_must) || (perm & ~mask_must & ~mask_may))
		return -E_INVAL;
...
sys_page_map(envid_t srcenvid, void *srcva, envid_t dstenvid, void *dstva, int perm)
{
	...
	struct Env *srcenv;
	int error = envid2env(srcenvid, &srcenv, false);
	if (error < 0)
		return error;
	...
	struct Env *dstenv;
	error = envid2env(dstenvid, &dstenv, false);
	if (error < 0)
		return error;

	int insert_result =
	        page_insert(dstenv->env_pgdir, page, dstva, perm | PTE_P | PTE_U);
	if (insert_result < 0)
		page_free(page);
