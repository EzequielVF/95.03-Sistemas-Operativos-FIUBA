...
extern int status;  // tuve que traerla para expandirla
...
expand_environ_var(char *arg)
{
	...
	if (strcmp(arg, "$?") != 0) {
		...
		char buffer[BUFLEN];
		snprintf(buffer, BUFLEN, "%d", status);
		aux = buffer;
	}
	if (!aux) {
		arg[0] = END_STRING;
	...
	size_t len_val = strlen(aux);
	if (len_arg < len_val) {
		arg = realloc(arg,
		              (len_val +
		               1) * sizeof(char));  // Acomodo la memoria reservada para que entre todo.
	}
	strcpy(arg, aux);
	return arg;
}
...
parse_line(char *buf)
{
	...
	if (strlen(right) > 0) {
		r = parse_line(right);
	}  // De esta forma logro hacer crecer el arbol hacia los hijos derechos
	   // de forma recursiva en el caso de tuberias multiples.
